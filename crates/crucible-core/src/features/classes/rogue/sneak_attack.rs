...
/// advantage/disadvantage, and [`crate::rules::Attack::ally_adjacent`]
/// standing in for the "an ally is next to the target" clause the engine has
/// no positioning model to derive (see `DESIGN.md`). Whoever builds the attack
/// for a given scenario or turn sets that flag the same way `mode` already
/// gets set; `Rider::extra_damage_for` is where the gate is actually checked.
///
...
pub(super) fn register(registry: &mut FeatureRegistry) {
    // Sneak Attack (2024 Rogue 1)
    registry.register("sneak_attack", |val| {
        let dice_count = val
            .get("dice_count")
...
mod tests {
    use super::*;
    use crate::prob::Rng;
    use crate::rules::{damage_pmf, sample_damage, Attack, DamageRider, Defense, RollMode};
